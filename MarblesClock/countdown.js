var RADIUS = 8; //球半径
var MARGIN_TOP = 60; //canvas上边距
var MARGIN_LEFT = 30; //canvas左边距
...
function updateBalls() {
    for (var i = 0; i < balls.length; i++) {
        ...
        if (balls[i].y >= WINDOW_HEIGHT - RADIUS) {
            ...
            balls[i].vy = -Math.abs(balls[i].vy) * 0.75; //弹跳高度 & 边界碰撞检测
        }
    }
    var cnt = 0
    for (var i = 0; i < balls.length; i++)
...
function updateTitle() {
    ...
    context.fillText(getCurrentDay(), MARGIN_LEFT * 0.15, WINDOW_HEIGHT * 0.15);
    context.closePath();
}
...
function getCurrentDay() {
    ...
    month[10] = "Nov"
    month[11] = "Dec"
    return month[date.getMonth()] + " " + date.getDate() + "th," + '\n' + weekday[date.getDay()] + date.getFullYear();
}